...
def dump_raw_data(raw_data, file_name, bos_index, eos_index):
...
def load_next_record(file_name):
    ...
    DIALOGUE = tf.cast(x=PARSED_RECORD["DIALOGUE"], dtype=tf.int32)
    BOS_LOCATIONS = tf.cast(x=PARSED_RECORD["BOS_LOCATIONS"], dtype=tf.int32)
    EOS_LOCATIONS = tf.cast(x=PARSED_RECORD["EOS_LOCATIONS"], dtype=tf.int32)

    return DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS


def get_encoder_batch(DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS):
    MAXIMUM_SIZE = tf.reduce_max(tf.subtract(x=EOS_LOCATIONS[:-1], y=BOS_LOCATIONS[:-1]))

    def SCAN_BATCH(ARGUMENT):
    ...
    ENCODER_SENTENCE_SIZE_BATCH, ENCODER_SENTENCE_UNIT_BATCH = tf.map_fn(
        fn=SCAN_BATCH,
        elems=[
            tf.pad(tensor=BOS_LOCATIONS[:-1], paddings=[[1, 0]]),
            tf.pad(tensor=EOS_LOCATIONS[:-1], paddings=[[1, 0]])
        ],
...
def get_decoder_batch(DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS):
    MAXIMUM_SIZE = tf.reduce_max(tf.subtract(x=EOS_LOCATIONS, y=BOS_LOCATIONS))
    ...
    DECODER_SENTENCE_SIZE_BATCH, DECODER_SENTENCE_UNIT_BATCH, DECODER_SENTENCE_LABEL_BATCH = tf.map_fn(
        fn=SCAN_BATCH,
        elems=[BOS_LOCATIONS, EOS_LOCATIONS],
        dtype=[tf.int32, tf.int32, tf.int32]
    )

    return DECODER_SENTENCE_SIZE_BATCH, DECODER_SENTENCE_UNIT_BATCH, DECODER_SENTENCE_LABEL_BATCH
...
def feed_forward(DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS):
    ...
    ENCODER_SENTENCE_SIZE_BATCH, ENCODER_SENTENCE_UNIT_BATCH = get_encoder_batch(
        DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS
    )

    DECODER_SENTENCE_SIZE_BATCH, DECODER_SENTENCE_UNIT_BATCH, DECODER_SENTENCE_LABEL_BATCH = get_decoder_batch(
        DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS
    )
    ...
    return FILTER_MASK, CLASS_SUCCESS, CROSS_ENTROPY, OVERALL_ENTROPY


def model_update(LEARNING_RATE, file_name, is_reuse):
    DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS = load_next_record(file_name)

    with tf.variable_scope(name_or_scope=tf.get_variable_scope(), reuse=is_reuse):
        FILTER_MASK, CLASS_SUCCESS, CROSS_ENTROPY, OVERALL_ENTROPY = feed_forward(
            DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS
        )

        OBJECTIVE = tf.divide(
            x=tf.reduce_sum(tf.multiply(x=FILTER_MASK, y=CROSS_ENTROPY)),
            y=tf.count_nonzero(input_tensor=FILTER_MASK, dtype=tf.float32)
        )
...
def model_assess(file_name, is_reuse):
    ...
    with tf.variable_scope(name_or_scope=tf.get_variable_scope(), reuse=is_reuse):
        ...
        FULL_COUNT = tf.count_nonzero(input_tensor=FILTER_MASK, dtype=tf.float32)
        FULL_LOSS = tf.reduce_sum(tf.multiply(x=FILTER_MASK, y=CROSS_ENTROPY))
        FULL_HIT = tf.reduce_sum(tf.multiply(x=FILTER_MASK, y=CLASS_SUCCESS))
        TAIL_COUNT = tf.count_nonzero(input_tensor=FILTER_MASK[-1], dtype=tf.float32)
        TAIL_LOSS = tf.reduce_sum(tf.multiply(x=FILTER_MASK[-1], y=CROSS_ENTROPY[-1]))
        TAIL_HIT = tf.reduce_sum(tf.multiply(x=FILTER_MASK[-1], y=CLASS_SUCCESS[-1]))
...
def model_predict(DIALOGUE, BOS_LOCATIONS, EOS_LOCATIONS, is_reuse):
    with tf.variable_scope(name_or_scope=tf.get_variable_scope(), reuse=is_reuse):
        ...
        RESPONSE_SIZE = tf.count_nonzero(input_tensor=FILTER_MASK[-1], dtype=tf.int32)
...
def run_update(SESS, UPDATE, LEARNING_RATE, learning_rate, file_name):
    for _ in range(get_record_count(file_name)):
